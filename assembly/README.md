...
The base plate is fabricated by laser cutting a 1/8th inch clear acrylic sheet using the file [here](https://github.com/BerkeleyAutomation/sprk/blob/master/assembly/BasePlate.AI). Additional holes can be made onto this base plate to fixture it to the working table. We are going to attach the servos directly onto this base plate, and need to add a countersink to the servo attach holes in order to use the Robotis Rivets. These counter sinks on the underside of the base plate can be made using any larger diameter drill bit on a drill press. The depth of the countersink can be up to 60 thou of an inch to firmly fix the servo onto the plate. The exact depth can be found by gradually increasing countersink depth till the rivets reach the servo. The countersinks also help to keep the rivets on the bottom flush with the surface of the base plate.

## Fabrication of the servo horns
The servo horns are fabricated using 1/4th inch Delrin sheets. Each horn has 4 holes to fix them to the servos using the Robotis rivets, and 1 hole to fix one end of the M3 nylon ball joint rods. Once these horns are laser cut using the files [here](https://github.com/BerkeleyAutomation/sprk/blob/master/assembly/ServoHorn.AI), they need to be post processed on a drill press. The rivets are not very long, so a countersink also needs to be made to allow the rivets to reach the servo. Note that the default servo horn was not removed and the Delrin horn was simply attached to it using the rivets. The exact counter sink depth on the Delrin horn can be found similarly to that with the base plate. Approximately, this depth can be up to 0.15 inch. The fifth hole – for the Nylon ball joint rod – can be drilled for an M3 screw thread. We use Delrin for the servo horn to be able to thread this hole. Repeat this process for 6 servo horns.
...
The top plate is also laser cut from a 1/8th inch acrylic sheet using the file [here](https://github.com/BerkeleyAutomation/sprk/blob/master/assembly/TopPlate.ai). The plate has three holes for attaching mounts for different subtasks. More holes can be added for more complex mounts. The Delrin buttress which is laser cut from 1/4th inch Delrin sheet using the file [here](https://github.com/BerkeleyAutomation/sprk/blob/master/assembly/Delrin_Buttress_BallJointHousing.ai) will help attach the Nylon ball joint rod to the top plate. So, we need to drill the two holes on each buttress for an M3 thread after laser cutting. Once this is completed for all 3 buttresses, we can press fit each of them into the top plate.
